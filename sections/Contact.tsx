...
export default function Contact() {
  return (
    <section
      id="contact"
      className="bg-black py-20 md:py-36 px-4 sm:px-6 md:px-12 border-t border-zinc-900 overflow-hidden relative"
    >
      <div className="max-w-7xl mx-auto w-full flex flex-col items-center text-center relative z-10 select-none">

        {/* Sub-label */}
        <span className="text-[11px] font-mono text-zinc-500 uppercase tracking-[0.3em] sm:tracking-[0.4em] mb-6">
          [ GET IN TOUCH ]
        </span>
        ...
        >
          <a
            ...
            className="font-syne font-black text-contact-heading uppercase text-white hover:text-zinc-300 transition-colors duration-500 inline-block group relative text-center"
          >
        ...
        <div className="w-full max-w-4xl grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mt-16 sm:mt-24 border-t border-zinc-900 pt-12 sm:pt-16 text-left">
          {socialLinks.map((link, index) => (
            <motion.a
              ...
              className="group flex flex-col space-y-2 py-3 sm:py-4 border-b border-transparent hover:border-zinc-800 transition-all duration-300"
            >
              <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">
                {link.name}
              </span>
              <span className="text-xs sm:text-sm font-syne font-bold text-white group-hover:text-zinc-400 transition-colors duration-300 relative inline-block overflow-hidden py-1 break-all sm:break-normal">
                {link.val}
